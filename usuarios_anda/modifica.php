...
switch($_REQUEST["accion"])
{
	...
	{	
		...
		$vector = consultaBD("usuarios_anda","dni='".$_POST["dni"]."' AND borrado=0");
		...
		insertaBD("usuarios_anda",$vector);		
	...
	case 'modificar':
	{
		...
		// Comprobamos si existe ya el nombre de usuario		
		$vector = consultaBD("usuarios_anda","dni='".$_POST["dni"]."' AND borrado=0 AND id_usuario <> ".intval($_POST["id_usuario"]));
		if (count($vector) > 0){ echo "5"; exit();};								
					
	
		
		$vector_anterior = array();
		$vector_anterior = consultaBD("usuarios_anda","dni=".intval($_POST["dni"]));
		...
		modificaBD("usuarios_anda",$vector,"id_usuario = ".intval($_POST["id_usuario"]));
	...
	case 'borrar':
	{
		...
		$vector_anterior = consultaBD("usuarios_anda","id_usuario=".intval($_GET["id_usuario"]));
	...
	//	borraBD("seguridad_usuarios","id_usuario=".$_GET["id_usuario"]);
		...
		modificaBD("usuarios_anda",$vector,$where);	
	...
	case 'cambiar_estado':
		{	
			...
			modificaBD("usuarios_anda",$vector,"id_usuario=".intval($_GET["id_usuario"]));
